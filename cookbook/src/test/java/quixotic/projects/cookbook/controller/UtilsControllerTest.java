...
public class UtilsControllerTest {
    ...
    @Test
    public void getPublicationTypes_returnsCorrectPublicationTypes() throws Exception {
        List<String> mockTypes = Arrays.asList("Type1", "Type2", "Type3");

        Mockito.when(utilsService.getPublicationType()).thenReturn(mockTypes);

        mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/utils/publication-types"))
                .andExpect(MockMvcResultMatchers.status().isAccepted())
                .andExpect(MockMvcResultMatchers.content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.content().json("[\"Type1\",\"Type2\",\"Type3\"]"));
    }
}